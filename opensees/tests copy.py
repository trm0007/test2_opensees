...
# Create a punching shear section
ps = PunchingShearSection(
    col_width=20,          # Column width (x-dimension)
    col_depth=20,          # Column depth (y-dimension)
    slab_avg_depth=8,      # Average slab depth
    condition="I",         # Interior condition
    studrail_length=12     # Stud rail length
)

# Add an opening
ps.add_opening(xo=15, yo=15, width=10, depth=10)

# Solve for given loads
results = ps.solve(
    Vz=-200,       # Vertical load (negative = downward)
    Mx=500,        # Moment about x-axis
    My=300,        # Moment about y-axis
    gamma_vx="auto",  # Auto-calculate moment transfer factors
    gamma_vy="auto",
    consider_ecc=True  # Consider eccentricity effects
)
...
os.makedirs("punching_shear", exist_ok=True)

# Generate and save 2D results plot
fig_2d = ps.plot_results()
fig_2d.savefig("punching_shear/punching_shear_2d.png", dpi=300, bbox_inches='tight')
plt.close(fig_2d)  # Close the figure to free memory

# Generate and save 3D interactive plot
fig_3d = ps.plot_results_3D()
fig_3d.write_html("punching_shear/punching_shear_3d.html")

# For the preview, we need to handle it differently since preview() doesn't return a figure
plt.figure()  # Create a new figure
ps.preview()  # This shows the preview
plt.savefig("punching_shear/perimeter_preview.png", dpi=300, bbox_inches='tight')
plt.close()  # Close the figure

# Save the results dataframe to CSV
results = ps.df_perimeter
results.to_csv("punching_shear/results.csv", index=False)
# Method 1: Using pandas to_json() - Creates properly formatted JSON
results.to_json("punching_shear/results.json", orient='records', indent=2)

# Method 2: Alternative - Convert to dict first, then save with json module
# This gives you more control over the JSON formatting
results_dict = results.to_dict('records')
with open("punching_shear/results.json", 'w') as f:
    json.dump(results_dict, f, indent=2)
print("All results saved to 'punching_shear' folder:")
print(f"- punching_shear_2d.png (2D results plot)")
print(f"- punching_shear_3d.html (interactive 3D plot)")
print(f"- perimeter_preview.png (geometry preview)")
print(f"- results.csv (detailed calculations)")
...
def evaluate_punching_shear(ps, f_prime_c=4000, beta=1.0, alpha_s=40, fy=60000, reinforcement_type='both'):
    """Enhanced safety evaluation with both conventional and stud reinforcement design"""
    phi = 0.75
    lambda_ = 1.0
    ...
    # Initialize reinforcement designs
    conventional_design = None
    stud_design = None
    
    # Safety evaluation and reinforcement design
    if v_max <= phi * vc:
        ...
        action = "Choose between conventional reinforcement or studs (see options below)"
        
        # Calculate both reinforcement options
        ...
        action = "Increase thickness, add drop panels, or increase f'c"
...
# Example usage:
ps = PunchingShearSection(col_width=20, col_depth=20, slab_avg_depth=8,
                          condition="I", studrail_length=12)

# Debug: Print available attributes
print("Available attributes in PunchingShearSection:")
print([attr for attr in dir(ps) if not attr.startswith('_')])

try:
    # Solve the punching shear analysis
    results = ps.solve(Vz=-250, Mx=500, My=300)
    
    # Perform safety evaluation with both reinforcement options
    evaluation = evaluate_punching_shear(ps, f_prime_c=4000, fy=60000, reinforcement_type='both')
    ...
    results_df = ps.df_perimeter
    results_df.to_json("punching_shear/results.json", orient='records', indent=2)
    
    # Save evaluation results
    with open("punching_shear/evaluation.json", 'w') as f:
        json.dump(evaluation, f, indent=2, default=str)
    
    # Print evaluation
    print("\n=== PUNCHING SHEAR EVALUATION ===")
    print(f"Perimeter b0: {evaluation['b0']:.1f} in")
    print(f"Effective depth d: {ps.slab_depth:.1f} in")
    print(f"Max Shear Stress: {evaluation['v_max']:.0f} psi")
    print(f"ϕVc: {evaluation['phiVc']:.0f} psi")
    print(f"D/C (concrete only): {evaluation['dc_concrete']:.2f}")
    print(f"Status: {evaluation['status']}")
    print(f"Action: {evaluation['action']}")
    
    # Print conventional reinforcement design if applicable
    if evaluation['conventional_design']:
        design = evaluation['conventional_design']
        if design['feasible']:
            print(f"\n=== OPTION 1: CONVENTIONAL REINFORCEMENT ===")
            print(f"Required Vs: {design['vs_required']:.0f} psi")
            print(f"Provided Vs: {design['vs_provided']:.0f} psi")
            print(f"Bar Size: {design['bar_size']}")
            print(f"Spacing Both Ways: {design['spacing_both_ways']}\" c/c")
            print(f"Maximum Allowed Spacing: {design['max_spacing_allowed']:.1f}\"")
            print(f"Critical Zone: {design['critical_width']:.1f}\" × {design['critical_depth']:.1f}\"")
            print(f"Bars in X-direction: {design['bars_x_direction']} bars @ {design['bar_length_x']:.1f}\" long")
            print(f"Bars in Y-direction: {design['bars_y_direction']} bars @ {design['bar_length_y']:.1f}\" long")
            print(f"Total Bars: {design['total_bars']}")
            print(f"Total Steel Area: {design['total_steel_area']:.2f} sq in")
            print(f"Design Summary: {design['message']}")
        else:
            print(f"\n=== CONVENTIONAL REINFORCEMENT NOT FEASIBLE ===")
            print(design['message'])
    
    # Print stud reinforcement design if applicable
    if evaluation['stud_design']:
        design = evaluation['stud_design']
        if design['feasible']:
            print(f"\n=== OPTION 2: STUD RAIL REINFORCEMENT ===")
            print(f"Required Vs: {design['vs_required']:.0f} psi")
            print(f"Provided Vs: {design['vs_provided']:.0f} psi")
            print(f"Maximum Vs: {design['vs_max']:.0f} psi")
            print(f"Stud Size: {design['stud_size']}")
            print(f"Studs per Row: {design['n_studs_per_row']}")
            print(f"Number of Rows: {design['n_rows']}")
            print(f"Total Studs: {design['total_studs']}")
            print(f"Tangential Spacing: {design['s_tangential']:.1f} in (max {design['s_max_tangential']:.1f} in)")
            print(f"Radial Spacing: {design['s_radial']:.1f} in maximum")
            print(f"Design Summary: {design['message']}")
        else:
            print(f"\n=== STUD REINFORCEMENT NOT FEASIBLE ===")
            print(design['message'])
    
    # Print comparison if both options are feasible
    if (evaluation['conventional_design'] and evaluation['conventional_design']['feasible'] and 
        evaluation['stud_design'] and evaluation['stud_design']['feasible']):
        print(f"\n=== DESIGN COMPARISON ===")
        conv = evaluation['conventional_design']
        stud = evaluation['stud_design']
        print(f"Conventional: {conv['total_bars']} bars, {conv['bar_size']} @ {conv['spacing_both_ways']}\" c/c both ways")
        print(f"Stud Rails:   {stud['total_studs']} studs, {stud['stud_size']}")
        print(f"Both options provide adequate capacity. Choose based on:")
        print(f"- Construction preference")
        print(f"- Material availability") 
        print(f"- Cost considerations")
    
    print(f"\n=== ANALYSIS SUMMARY ===")
    print(f"Number of segments: {len(results_df)}")
    print(f"Min shear stress: {results_df['v_total'].min():.4f} ksi")
    print(f"Max shear stress: {results_df['v_total'].max():.4f} ksi")
