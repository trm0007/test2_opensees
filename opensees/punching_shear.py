# read this code and give example plot all images and also return all results import time
import json
import pandas as pd
import numpy as np
import time
...
def calculate_column_capital_design(ps, v_max, vc_original, f_prime_c=4000):
    """
    Calculate required column capital (drop panel) to resist punching shear
    
    Parameters:
    - ps: PunchingShearSection object
    - v_max: maximum shear stress (psi)
    - vc_original: original concrete capacity without capital (psi)
    - f_prime_c: concrete compressive strength (psi)
    
    Returns:
    - Dictionary with column capital design
    """
    phi = 0.75
    lambda_ = 1.0
    
    # Get current section properties
    results_df = ps.df_perimeter
    b0_original = results_df['length'].sum()
    d_original = ps.slab_depth
    
    # Get column dimensions
    if hasattr(ps, 'col_width'):
        col_width = ps.col_width
        col_depth = ps.col_depth
    elif hasattr(ps, 'column_width'):
        col_width = ps.column_width
        col_depth = ps.column_depth
    elif hasattr(ps, 'c1') and hasattr(ps, 'c2'):
        col_width = ps.c1
        col_depth = ps.c2
    else:
        col_perimeter = min(results_df['length'].sum() / 4, 20)
        col_width = col_depth = col_perimeter
    
    # Required capacity
    vu_required = v_max / phi  # psi
    
    # Option 1: Increase slab thickness (drop panel)
    # Try different thickness increases
    thickness_options = []
    for extra_thickness in [2, 3, 4, 6, 8]:
        d_new = d_original + extra_thickness
        # Recalculate vc with new depth (assuming same perimeter for simplification)
        vc1 = 4 * lambda_ * np.sqrt(f_prime_c)
        vc2 = (2 + 4/1.0) * lambda_ * np.sqrt(f_prime_c)  # Assuming beta = 1.0
        vc3 = (40 * d_new / b0_original + 2) * lambda_ * np.sqrt(f_prime_c)  # Assuming alpha_s = 40
        vc_new = min(vc1, vc2, vc3)
        
        if phi * vc_new >= v_max:
            thickness_options.append({
                'extra_thickness': extra_thickness,
                'total_thickness': d_original + extra_thickness,
                'vc_new': vc_new,
                'phi_vc_new': phi * vc_new,
                'dc_ratio': v_max / (phi * vc_new),
                'feasible': True
            })
    
    # Option 2: Column capital (expanded column head)
    capital_options = []
    for capital_extension in [6, 12, 18, 24, 30]:  # inches beyond column face
        # New effective column dimensions
        col_width_eff = col_width + 2 * capital_extension
        col_depth_eff = col_depth + 2 * capital_extension
        
        # New critical perimeter (approximation)
        b0_new = 2 * (col_width_eff + col_depth_eff) + 2 * np.pi * d_original
        
        # Recalculate vc with new perimeter
        vc1 = 4 * lambda_ * np.sqrt(f_prime_c)
        vc2 = (2 + 4/1.0) * lambda_ * np.sqrt(f_prime_c)
        vc3 = (40 * d_original / b0_new + 2) * lambda_ * np.sqrt(f_prime_c)
        vc_new = min(vc1, vc2, vc3)
        
        if phi * vc_new >= v_max:
            capital_options.append({
                'capital_extension': capital_extension,
                'capital_width': col_width_eff,
                'capital_depth': col_depth_eff,
                'b0_new': b0_new,
                'vc_new': vc_new,
                'phi_vc_new': phi * vc_new,
                'dc_ratio': v_max / (phi * vc_new),
    ...
    # Option 3: Combined drop panel + capital
    combined_options = []
    for thickness_add in [2, 4]:
        for capital_ext in [12, 18]:
            d_new = d_original + thickness_add
            col_width_eff = col_width + 2 * capital_ext
            col_depth_eff = col_depth + 2 * capital_ext
            b0_new = 2 * (col_width_eff + col_depth_eff) + 2 * np.pi * d_new
            
            vc1 = 4 * lambda_ * np.sqrt(f_prime_c)
            vc2 = (2 + 4/1.0) * lambda_ * np.sqrt(f_prime_c)
            vc3 = (40 * d_new / b0_new + 2) * lambda_ * np.sqrt(f_prime_c)
            vc_new = min(vc1, vc2, vc3)
            
            if phi * vc_new >= v_max:
                combined_options.append({
                    'extra_thickness': thickness_add,
                    'capital_extension': capital_ext,
                    'total_thickness': d_new,
                    'capital_width': col_width_eff,
                    'capital_depth': col_depth_eff,
                    'b0_new': b0_new,
                    'vc_new': vc_new,
    ...
    return {
        ...
        'original_thickness': d_original,
        'original_perimeter': b0_original,
        'original_column': f"{col_width}\" × {col_depth}\"",
        'feasible': len(thickness_options) > 0 or len(capital_options) > 0 or len(combined_options) > 0
    }

def calculate_concrete_strength_upgrade(ps, v_max, vc_original, current_fc=4000):
    """
    Calculate required concrete strength increase to resist punching shear
    """
    phi = 0.75
    lambda_ = 1.0
    ...
    results_df = ps.df_perimeter
    b0 = results_df['length'].sum()
    d = ps.slab_depth
    
    # Required vc
    vc_required = v_max / phi
    
    # Try different concrete strengths
    fc_options = [5000, 6000, 7000, 8000, 10000]  # psi
    strength_options = []
    
    for fc_new in fc_options:
        if fc_new <= current_fc:
            continue
            
        # Recalculate vc with new f'c
        vc1 = 4 * lambda_ * np.sqrt(fc_new)
        vc2 = (2 + 4/1.0) * lambda_ * np.sqrt(fc_new)
        vc3 = (40 * d / b0 + 2) * lambda_ * np.sqrt(fc_new)
        vc_new = min(vc1, vc2, vc3)
        
        if phi * vc_new >= v_max:
            strength_options.append({
                'fc_new': fc_new,
                'fc_increase': fc_new - current_fc,
                'vc_new': vc_new,
                'phi_vc_new': phi * vc_new,
                'dc_ratio': v_max / (phi * vc_new),
                'strength_increase_percent': ((fc_new - current_fc) / current_fc) * 100,
                'feasible': True
            })
    ...
    return {
        ...
        'feasible': len(strength_options) > 0
    }
    

def calculate_conventional_reinforcement(ps, v_max, vc, f_prime_c=4000, fy=60000):
    """
    Calculate required conventional reinforcement (rebar in both directions) per ACI 318
    
    Parameters:
    - ps: PunchingShearSection object
    - v_max: maximum shear stress (psi)
    - vc: concrete shear capacity (psi)
    - f_prime_c: concrete compressive strength (psi)
    - fy: yield strength of reinforcement (psi)
    
    Returns:
    - Dictionary with conventional reinforcement requirements
    """
    phi = 0.75
    lambda_ = 1.0
    
    # Get critical section properties
    results_df = ps.df_perimeter
    b0 = results_df['length'].sum()  # Critical perimeter
    d = ps.slab_depth
    
    # Required shear to be carried by reinforcement
    vs_required = (v_max / phi) - vc  # psi
    
    # Maximum allowable Vs per ACI 318
    vs_max = 6 * lambda_ * np.sqrt(f_prime_c)  # psi (lower for conventional reinforcement)
    
    if vs_required > vs_max:
        return {
            'feasible': False,
            'vs_required': vs_required,
            'vs_max': vs_max,
            'message': f'Required Vs ({vs_required:.0f} psi) exceeds maximum allowed for conventional reinforcement ({vs_max:.0f} psi). Consider studs or increase slab thickness.'
        }
    
    # Calculate required reinforcement area
    # For conventional reinforcement: Vs = (Av * fy * sin(α)) / (s1 * s2)
    # Where α = 90° for perpendicular bars, so sin(α) = 1
    # Av = total area of reinforcement crossing the failure surface
    
    # Approximate critical zone dimensions (typically column + 3d on each side)
    ...
    if hasattr(ps, 'col_width'):
        ...
        # Estimate from perimeter if column dimensions not available
        # Assume square column for simplification
        col_perimeter = min(results_df['length'].sum() / 4, 20)  # Conservative estimate
        col_width = col_depth = col_perimeter
        print(f"Warning: Column dimensions not found, estimated as {col_width:.1f}\" square")
    
    critical_width = col_width + 6*d  # 3d on each side
    critical_depth = col_depth + 6*d  # 3d on each side
    
    # Required reinforcement area per unit width for each direction
    # Vs = (As_per_width * fy) where As_per_width is area per unit width
    As_req_per_width = vs_required / fy  # sq in per inch width
    ...
    # Select appropriate bar size (typically #4 or #5 for punching shear)
    selected_bar = '#5'  # Can be modified based on preference
    bar_area = bar_sizes[selected_bar]['area']
    
    # Calculate spacing for each direction
    # Required spacing = bar_area / As_req_per_width
    spacing_required = bar_area / As_req_per_width if As_req_per_width > 0 else 12
    
    # ACI 318 maximum spacing requirements for shear reinforcement
    max_spacing = min(d/2, 24)  # inches
    
    # Use the more restrictive spacing
    spacing_use = min(spacing_required, max_spacing)
    
    # Round down to practical spacing (3", 4", 6", 8", 12")
    practical_spacings = [3, 4, 5, 6, 7, 8, 9, 10, 12]
    spacing_final = max([s for s in practical_spacings if s <= spacing_use], default=3)
    
    # Calculate actual provided reinforcement
    As_provided_per_width = bar_area / spacing_final
    vs_provided = As_provided_per_width * fy
    
    # Calculate number of bars needed in each direction
    bars_x_direction = int(np.ceil(critical_width / spacing_final)) + 1
    bars_y_direction = int(np.ceil(critical_depth / spacing_final)) + 1
    
    # Total reinforcement area
    total_bars = bars_x_direction + bars_y_direction
    total_steel_area = total_bars * bar_area
    
    # Length of bars (extend beyond critical section)
    bar_length_x = critical_width + 2*d  # Development length consideration
    bar_length_y = critical_depth + 2*d
    
    return {
        'feasible': True,
        'vs_required': vs_required,
        'vs_provided': vs_provided,
        'vs_max': vs_max,
        'bar_size': selected_bar,
        'bar_area': bar_area,
        'spacing_both_ways': spacing_final,
        'max_spacing_allowed': max_spacing,
        'bars_x_direction': bars_x_direction,
        'bars_y_direction': bars_y_direction,
        'total_bars': total_bars,
        'bar_length_x': bar_length_x,
        'bar_length_y': bar_length_y,
        'total_steel_area': total_steel_area,
        'critical_width': critical_width,
        'critical_depth': critical_depth,
        'message': f'Use {selected_bar} bars @ {spacing_final}" c/c both ways in critical zone'
    }

def calculate_stud_reinforcement(ps, v_max, vc, f_prime_c=4000, fy=60000):
    """
    Calculate required shear reinforcement (stud rails) per ACI 318
    ...
    Returns:
    - Dictionary with reinforcement requirements
    """
    phi = 0.75
    lambda_ = 1.0
    ...
    # Maximum allowable Vs per ACI 318
    vs_max = 8 * lambda_ * np.sqrt(f_prime_c)  # psi
    ...
    if vs_required > vs_max:
        return {
            ...
            'vs_required': vs_required,
            'vs_max': vs_max,
            'message': f'Required Vs ({vs_required:.0f} psi) exceeds maximum allowed ({vs_max:.0f} psi). Increase slab thickness or concrete strength.'
        }
    
    # Stud rail design parameters
    # Typical stud rail: #4 bar (0.2 sq in), but can be adjusted
    stud_diameter = 0.5  # inches (#4 bar)
    As_stud = 0.2  # sq in per stud (#4 bar)
    
    # Required area of shear reinforcement per unit length of perimeter
    # Vs = (Av * fy * d) / s_radial, where s_radial is spacing in radial direction
    # For punching shear: Vs = (Av * fy) / s_tangential (per unit length of perimeter)
    
    # Calculate required Av per unit length of perimeter (sq in per inch)
    Av_per_length = vs_required / fy  # sq in per inch of perimeter
    
    # Total required Av around the perimeter
    Av_total = Av_per_length * b0  # sq in
    
    # Number of studs required (minimum)
    n_studs_min = Av_total / As_stud
    
    # Spacing considerations per ACI 318
    # Maximum spacing in tangential direction: d/2
    s_max_tangential = d / 2  # inches
    
    # Calculate actual spacing based on perimeter
    s_tangential = b0 / n_studs_min if n_studs_min > 0 else b0
    
    # Check if spacing is acceptable
    if s_tangential > s_max_tangential:
        # Need more studs to meet spacing requirements
        n_studs_spacing = np.ceil(b0 / s_max_tangential)
        n_studs_required = max(n_studs_min, n_studs_spacing)
        s_tangential_actual = b0 / n_studs_required
    else:
        n_studs_required = np.ceil(n_studs_min)
        s_tangential_actual = b0 / n_studs_required
    
    # Radial spacing (multiple rows of studs)
    # First row at d/2 from column face
    # Subsequent rows at spacing not exceeding 0.75d
    s_max_radial = 0.75 * d
    
    # Calculate number of radial rows needed
    # Critical section is typically at d/2 from column face
    # Shear reinforcement extends to where Vc alone is sufficient
    
    # Distance where Vc is sufficient (approximate)
    # This is where v_actual <= phi * vc
    # For simplification, assume 2-3 rows are typically needed
    n_rows = 2  # Can be refined based on more detailed analysis
    
    # Total number of studs
    total_studs = int(n_studs_required * n_rows)
    
    # Provided capacity
    vs_provided = (n_studs_required * As_stud * fy) / b0
    ...
    return {
        ...
        'vs_provided': vs_provided,
        'vs_max': vs_max,
        'n_studs_per_row': int(n_studs_required),
        'n_rows': n_rows,
        'total_studs': total_studs,
        's_tangential': s_tangential_actual,
        's_max_tangential': s_max_tangential,
        's_radial': s_max_radial,
        'stud_size': f"#{int(stud_diameter*8)} ({stud_diameter}\" dia)",
        'As_per_stud': As_stud,
        'perimeter': b0,
        'message': f'Use {total_studs} studs total: {int(n_studs_required)} studs per row × {n_rows} rows'
    }


def evaluate_punching_shear(ps, f_prime_c=4000, beta=1.0, alpha_s=40, fy=60000, reinforcement_type='both'):
    """Enhanced safety evaluation with multiple design options"""
    phi = 0.75
    lambda_ = 1.0
    ...
    # Calculate perimeter b0 from the results
    b0 = results_df['length'].sum()
    d = ps.slab_depth
    
    # Find maximum shear stress from results
    v_max = abs(results_df['v_total'].min()) * 1000  # Convert to psi
    
    # Calculate concrete capacity
    vc1 = 4 * lambda_ * np.sqrt(f_prime_c)
    vc2 = (2 + 4/beta) * lambda_ * np.sqrt(f_prime_c)
    vc3 = (alpha_s * d / b0 + 2) * lambda_ * np.sqrt(f_prime_c)
    vc = min(vc1, vc2, vc3)
    
    # Calculate existing Vs if studrails exist
    vs_existing = 50 * (ps.L_studrail / d) if ps.has_studrail else 0
    vn_existing = vc + vs_existing
    
    # Demand-capacity ratios
    dc_concrete = v_max / (phi * vc)
    dc_total = v_max / (phi * vn_existing) if ps.has_studrail else dc_concrete
    
    # Initialize design options
    conventional_design = None
    stud_design = None
    capital_design = None
    strength_upgrade = None
    
    # Safety evaluation and design options
    if v_max <= phi * vc:
        status = "SAFE - No shear reinforcement needed"
        action = "Design adequate"
    elif v_max <= phi * (vc + 8 * lambda_ * np.sqrt(f_prime_c)):  # Check if reinforcement is feasible
        status = "SAFE - With shear reinforcement"
        action = "Choose between reinforcement options (see below)"
        
        # Calculate reinforcement options
        if reinforcement_type in ['both', 'conventional']:
            conventional_design = calculate_conventional_reinforcement(ps, v_max, vc, f_prime_c, fy)
        
        if reinforcement_type in ['both', 'studs']:
            stud_design = calculate_stud_reinforcement(ps, v_max, vc, f_prime_c, fy)
        
        # Calculate capital design options
        capital_design = calculate_column_capital_design(ps, v_max, vc, f_prime_c)
        
        # Calculate concrete strength upgrade options
        strength_upgrade = calculate_concrete_strength_upgrade(ps, v_max, vc, f_prime_c)
    else:
        status = "UNSAFE - Redesign required"
        action = "Consider multiple redesign options (see below)"
        
        # Calculate all possible redesign options since section is unsafe
        capital_design = calculate_column_capital_design(ps, v_max, vc, f_prime_c)
        strength_upgrade = calculate_concrete_strength_upgrade(ps, v_max, vc, f_prime_c)
        
        # Still calculate reinforcement options in case they can help
        if reinforcement_type in ['both', 'conventional']:
            conventional_design = calculate_conventional_reinforcement(ps, v_max, vc, f_prime_c, fy)
    ...
    result = {
        'v_max': v_max,
        'phiVc': phi * vc,
        'phiVn_existing': phi * vn_existing,
        'dc_concrete': dc_concrete,
        'dc_total': dc_total,
        'status': status,
        'action': action,
        'b0': b0,
        ...
        'stud_design': stud_design,
        'capital_design': capital_design,
...
def save_punching_shear_report(evaluation, ps, results_df):
    # Ensure directory exists
    output_dir = "punching_shear"
    os.makedirs(output_dir, exist_ok=True)
    filepath = os.path.join(output_dir, "punching_shear_report.txt")

    lines = []

    # Evaluation
    lines.append("=== PUNCHING SHEAR EVALUATION ===")
    lines.append(f"Perimeter b0: {evaluation['b0']:.1f} in")
    lines.append(f"Effective depth d: {ps.slab_depth:.1f} in")
    lines.append(f"Max Shear Stress: {evaluation['v_max']:.0f} psi")
    lines.append(f"ϕVc: {evaluation['phiVc']:.0f} psi")
    lines.append(f"D/C (concrete only): {evaluation['dc_concrete']:.2f}")
    lines.append(f"Status: {evaluation['status']}")
    lines.append(f"Action: {evaluation['action']}")

    # Conventional reinforcement
    design = evaluation.get('conventional_design')
    if design:
        if design['feasible']:
            lines.append("\n=== OPTION 1: CONVENTIONAL REINFORCEMENT ===")
            lines.append(f"Required Vs: {design['vs_required']:.0f} psi")
            lines.append(f"Provided Vs: {design['vs_provided']:.0f} psi")
            lines.append(f"Bar Size: {design['bar_size']}")
            lines.append(f"Spacing Both Ways: {design['spacing_both_ways']}\" c/c")
            lines.append(f"Maximum Allowed Spacing: {design['max_spacing_allowed']:.1f}\"")
            lines.append(f"Critical Zone: {design['critical_width']:.1f}\" × {design['critical_depth']:.1f}\"")
            lines.append(f"Bars in X-direction: {design['bars_x_direction']} bars @ {design['bar_length_x']:.1f}\" long")
            lines.append(f"Bars in Y-direction: {design['bars_y_direction']} bars @ {design['bar_length_y']:.1f}\" long")
            lines.append(f"Total Bars: {design['total_bars']}")
            lines.append(f"Total Steel Area: {design['total_steel_area']:.2f} sq in")
            lines.append(f"Design Summary: {design['message']}")
        else:
            lines.append("\n=== CONVENTIONAL REINFORCEMENT NOT FEASIBLE ===")
            lines.append(design['message'])

    # Stud reinforcement
    design = evaluation.get('stud_design')
    if design:
        if design['feasible']:
            lines.append("\n=== OPTION 2: STUD RAIL REINFORCEMENT ===")
            lines.append(f"Required Vs: {design['vs_required']:.0f} psi")
            lines.append(f"Provided Vs: {design['vs_provided']:.0f} psi")
            lines.append(f"Maximum Vs: {design['vs_max']:.0f} psi")
            lines.append(f"Stud Size: {design['stud_size']}")
            lines.append(f"Studs per Row: {design['n_studs_per_row']}")
            lines.append(f"Number of Rows: {design['n_rows']}")
            lines.append(f"Total Studs: {design['total_studs']}")
            lines.append(f"Tangential Spacing: {design['s_tangential']:.1f} in (max {design['s_max_tangential']:.1f} in)")
            lines.append(f"Radial Spacing: {design['s_radial']:.1f} in maximum")
            lines.append(f"Design Summary: {design['message']}")
        else:
            lines.append("\n=== STUD REINFORCEMENT NOT FEASIBLE ===")
            lines.append(design['message'])

    # Design comparison
    conv = evaluation.get('conventional_design')
    stud = evaluation.get('stud_design')
    if conv and conv['feasible'] and stud and stud['feasible']:
        lines.append("\n=== DESIGN COMPARISON ===")
        lines.append(f"Conventional: {conv['total_bars']} bars, {conv['bar_size']} @ {conv['spacing_both_ways']}\" c/c both ways")
        lines.append(f"Stud Rails:   {stud['total_studs']} studs, {stud['stud_size']}")
        lines.append(f"Both options provide adequate capacity. Choose based on:")
        lines.append(f"- Construction preference")
        lines.append(f"- Material availability")
        lines.append(f"- Cost considerations")

    # Capital design
    capital = evaluation.get('capital_design')
    if capital:
        lines.append("\n=== OPTION 3: COLUMN CAPITAL/DROP PANEL DESIGN ===")
        lines.append(f"Original Column: {capital['original_column']}")
        lines.append(f"Original Thickness: {capital['original_thickness']}\"")
        lines.append(f"Original Perimeter: {capital['original_perimeter']:.1f}\"")

        if capital['thickness_options']:
            lines.append("\nDrop Panel Thickness Options:")
            for i, opt in enumerate(capital['thickness_options'], 1):
                lines.append(f"{i}. Add {opt['extra_thickness']}\" thickness (Total: {opt['total_thickness']}\")")
                lines.append(f"   New ϕVc: {opt['phi_vc_new']:.0f} psi, D/C: {opt['dc_ratio']:.2f}")

        if capital['capital_options']:
            lines.append("\nColumn Capital Extension Options:")
            for i, opt in enumerate(capital['capital_options'], 1):
                lines.append(f"{i}. Extend {opt['capital_extension']}\" beyond column face")
                lines.append(f"   New Capital: {opt['capital_width']}\" × {opt['capital_depth']}\"")
                lines.append(f"   New Perimeter: {opt['b0_new']:.1f}\", New ϕVc: {opt['phi_vc_new']:.0f} psi, D/C: {opt['dc_ratio']:.2f}")

        if capital['combined_options']:
            lines.append("\nCombined Drop Panel + Capital Options:")
            for i, opt in enumerate(capital['combined_options'], 1):
                lines.append(f"{i}. Add {opt['extra_thickness']}\" thickness + {opt['capital_extension']}\" capital extension")
                lines.append(f"   Total Thickness: {opt['total_thickness']}\", Capital: {opt['capital_width']}\" × {opt['capital_depth']}\"")
                lines.append(f"   New Perimeter: {opt['b0_new']:.1f}\", New ϕVc: {opt['phi_vc_new']:.0f} psi, D/C: {opt['dc_ratio']:.2f}")

        if not capital['feasible']:
            lines.append("\nNo feasible column capital/drop panel solutions found within typical limits")

    # Strength upgrade
    strength = evaluation.get('strength_upgrade')
    if strength:
        lines.append("\n=== OPTION 4: CONCRETE STRENGTH UPGRADE ===")
        lines.append(f"Current f'c: {strength['current_fc']} psi")
        if strength['strength_options']:
            lines.append("\nConcrete Strength Upgrade Options:")
            for i, opt in enumerate(strength['strength_options'], 1):
                lines.append(f"{i}. Increase f'c to {opt['fc_new']} psi (+{opt['fc_increase']} psi, +{opt['strength_increase_percent']:.0f}%)")
                lines.append(f"   New ϕVc: {opt['phi_vc_new']:.0f} psi, D/C: {opt['dc_ratio']:.2f}")
        else:
    ...
    # Summary
    lines.append("\n=== ANALYSIS SUMMARY ===")
    lines.append(f"Number of segments: {len(results_df)}")
    lines.append(f"Min shear stress: {results_df['v_total'].min():.4f} ksi")
    lines.append(f"Max shear stress: {results_df['v_total'].max():.4f} ksi")
...
def run_punching_shear_analysis(
    col_width, col_depth, slab_avg_depth,
    condition, studrail_length,
    Vz, Mx, My,
    openings=None,
    gamma_vx="auto", gamma_vy="auto",
    consider_ecc=True,
    f_prime_c=4000, fy=60000,
    reinforcement_type="both",
    output_folder="punching_case"
):
    base_folder = os.path.join("punching_shear", output_folder)
    os.makedirs(base_folder, exist_ok=True)

    ps = PunchingShearSection(
        col_width=col_width,
        col_depth=col_depth,
    ...
    results = ps.solve(
        Vz=Vz, Mx=Mx, My=My,
        gamma_vx=gamma_vx, gamma_vy=gamma_vy,
        consider_ecc=consider_ecc
    )

    # Save 2D plot
    fig_2d = ps.plot_results()
    fig_2d.savefig(os.path.join(base_folder, "punching_shear_2d.png"), dpi=300, bbox_inches='tight')
    plt.close(fig_2d)

    # Save 3D plot
    fig_3d = ps.plot_results_3D()
    fig_3d.write_html(os.path.join(base_folder, "punching_shear_3d.html"))

    # Save preview
    plt.figure()
    ps.preview()
    plt.savefig(os.path.join(base_folder, "perimeter_preview.png"), dpi=300, bbox_inches='tight')
    plt.close()

    # Save DataFrame
    results_df = ps.df_perimeter
    results_df.to_csv(os.path.join(base_folder, "results.csv"), index=False)
    results_df.to_json(os.path.join(base_folder, "results.json"), orient='records', indent=2)

    # Evaluation
    evaluation = evaluate_punching_shear(
        ps, f_prime_c=f_prime_c, fy=fy, reinforcement_type=reinforcement_type
    )

    # Save evaluation
    with open(os.path.join(base_folder, "evaluation.json"), 'w') as f:
        json.dump(evaluation, f, indent=2, default=str)

    # Save report
    save_punching_shear_report(evaluation, ps, results_df)

# cases = [
#     {
#         "col_width": 20, "col_depth": 20, "slab_avg_depth": 8,
#         "condition": "I", "studrail_length": 12,
#         "Vz": -200, "Mx": 500, "My": 300,
#         "openings": [{"xo": 15, "yo": 15, "width": 10, "depth": 10}],
#         "output_folder": "punching_case_1"
#     },
#     {
#         "col_width": 25, "col_depth": 20, "slab_avg_depth": 9,
#         "condition": "I", "studrail_length": 10,
#         "Vz": -250, "Mx": 600, "My": 400,
#         "openings": [],
#         "output_folder": "punching_case_2"
#     },
# ]

# for case in cases:
#     run_punching_shear_analysis(**case)
